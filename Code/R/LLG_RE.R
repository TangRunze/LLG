...
dataNameTitleVec = c("JHU, N=200", "Desikan, N=200", "CPAC200, N=200")
mVec = c(1, 5, 10)

for (iData in 1:length(dataNameVec)) {
  dataName1 <- dataNameVec[iData]
  dataName <- dataNameTitleVec[iData]
  for (iM in 1:length(mVec)) {
    ...
    if (dataName == "Desikan") {
      ...
      fileName = paste("../../Result/result_", dataName1, "_brute_", "m_", m, "_eig.RData", sep="")
    }
    load(fileName)
...
df_USVT <- list()
df_Abar <- list()
for (dataName in dataNameTitleVec) {
...
error_by_dim_df <- rbind(
  df_ZG[[dataNameTitleVec[1]]], df_ZG[[dataNameTitleVec[2]]], df_ZG[[dataNameTitleVec[3]]],
  df_USVT[[dataNameTitleVec[1]]], df_USVT[[dataNameTitleVec[2]]], df_USVT[[dataNameTitleVec[3]]],
  df_Abar[[dataNameTitleVec[1]]], df_Abar[[dataNameTitleVec[2]]], df_Abar[[dataNameTitleVec[3]]]) %>%
  mutate(dataname = factor(dataname, dataNameTitleVec))
...
ggsave("../../Draft/corr_data_REdiff.pdf",
