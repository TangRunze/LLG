...
These plots show the relative error of using a rank-$d$ approximation of $P$ (see Algorithm~\ref{algo:lowrank}) for three different atlases. In particular, this relative error of the rank-$d$ approximation happens to be the sum of absolute values of the smallest $N - d$ eigenvalues of $P$ divided by the sum of absolute values of all eigenvalues of $P$.
For all three atlases, the relative error is large unless $d$ is close to $N$, which indicates that $P$ is not low-rank.}
\label{fig:screeplot}
\end{figure}
