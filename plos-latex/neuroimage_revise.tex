...
In statistical decision theory, we say that a procedure $\delta^{\prime}$ dominates another procedure $\delta$ if the risk of $\delta^{\prime}$ is no bigger than the risk of $\delta$ regardless of the parameter, and strictly less than the risk of $\delta$ for in some situations.
...
\begin{figure}[!tbp]
\centering
\includegraphics[height=.2\textheight]{P_desikan_m5.pdf} \hspace{-35pt}
...
\includegraphics[height=.2\textheight]{P_desikan_m1.pdf} \hspace{-35pt}
...
The three heat maps in the upper level indicate the population mean for the $454$ graphs (Panel (a)), sample mean for the 5 sampled graphs (Panel (b)), and $\hat{P}$ for the same 5 sampled graphs with dimension $d=11$ selected using the Zhu and Ghodsi method (Panel (c)). Details about how to construct $\hat{P}$ are discussed in Section~\ref{sec:phat}.
...
Now take a sample of size $M=5$ in the CoRR dataset example with Desikan atlases mentioned above, then we calculate the entry-wise sample mean $\bar{A}$ and plot it in Panel (b) of Fig.~\ref{fig:Matrix_desikan_m5}. Darker pixels indicate a higher probability of an edge between the given vertices.
...
In Panel (c) of Fig.~\ref{fig:Matrix_desikan_m5}, we plot the heat map of $\bar{A}$ based on sample size $M = 1$. Since there is only one observed graph, $\bar{A}$ is binary and thus very bumpy. Similarly, we use the same absolute estimation error threshold $0.4$ and highlight 504 edges in the upper triangular.
...
Finally, Table~\ref{table:notation} presents the major symbols we use in the paper and their definitions.
...
$K$	& 	number of blocks of a stochastic blockmodel\\\hline
$G$	& 	random graph\\\hline
$P$	& 	$n \times n$ probability matrix \\\hline
$A$, $A^{(m)}$	&	$n \times n$ adjacency matrix (binary) \\\hline
$\bar{A}$	& 	$n \times n$ matrix of entry-wise sample mean of adjacency matrices\\\hline
...
$X$		& 	$n \times d$ matrix of latent positions, where the $i$-th row is $X_i^{\top}$\\\hline
$B$		& 	$K \times K$ matrix of block probability in a stochastic blockmodel \\\hline
$\tau$	& 	$n \times 1$ vector of block membership in a stochastic blockmodel \\\hline
$\nu_k$	& 	$d \times 1$ vector of latent position for the $k$-th block \\\hline
$\nu$	& 	$K \times d$ matrix of latent positions, where the $i$-th row is $\nu_k^{\top}$\\\hline
...
Low-rank methods can often be more easily interpreted from a vertex-level perspective.
...
As an examples, we consider the CoRR brain graphs with Desikan atlases.
More details about this dataset are given in Section \ref{sec:corr_data} and Section \ref{section:data}. 
By embedding the mean graph $P$, the average of all 454 graphs, we obtain estimated latent positions $\hat{X} \in \mathbb{R}^{N\times d}$, where $N=70$ is the number of vertices and $d = 8$ is the dimension selected by the Zhu and Ghodsi's method.
...
In section~\ref{section:lobe_structure} we analyze the structure of these latent positions as they relate to the lobe-structure of the brain.
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,width=.48\linewidth]{desikan1a.png}\hspace{-9pt}\\
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,width=.48\linewidth]{desikan1b.png}\hspace{-9pt}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,width=.48\linewidth]{desikan1c.png}
\end{center}
\end{subfigure}
% \vspace*{5pt}
\begin{subfigure}[t]{.48\textwidth}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,width=.48\linewidth]{desikan2a.png}\hspace{-10pt}\\
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,width=.48\linewidth]{desikan2b.png}\hspace{-10pt}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,width=.48\linewidth]{desikan2c.png}
\end{center}
\end{subfigure}\\
% \vspace*{5pt}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,height=.35\linewidth]{desikan3a.png}\hspace{-10pt}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,height=.35\linewidth]{desikan3b.png}\hspace{-10pt}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,height=.35\linewidth]{desikan3c.png}
\end{center}
\end{subfigure}\\
% \vspace*{5pt}
\begin{subfigure}[t]{.7\textwidth}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,height=.35\linewidth]{desikan4a.png}\hspace{-10pt}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,height=.35\linewidth]{desikan4b.png}\hspace{-10pt}
  \includegraphics[trim={5cm 5cm  4cm  4cm },clip,height=.35\linewidth]{desikan4c.png}
\end{center}
\end{subfigure}\\
% \vspace*{5pt}
\begin{subfigure}[t]{.7\textwidth}
...
\end{center}
\end{subfigure}\\
\caption{{\bf Brain plots colored by the first 5 dimensions of $\hat{X}$ for the Desikan atlas.}
% Bright red corresponds to large negative v
From the figure, we can see the embeddings reflect some well known structure.
For example, the second dimension displays a clear distinction of the left and right hemispheres.
% Also, the first dimension provides an average level of the entire brain.
In section~\ref{section:lobe_structure} we investigate how these latent positions correlate with lobe-structure, as can be seen in the 3rd and 4th dimensions where th red areas correspond to the {\color{red} blank and blank} lobes, respectively.}
...
\begin{subfigure}{.45\textwidth}
  \centering
  \includegraphics[height=\linewidth]{SBM_P.pdf}
\end{subfigure}%
\begin{subfigure}{.45\textwidth}
  \centering
  \includegraphics[height=\linewidth]{SBM_A.pdf}
\end{subfigure}
\begin{subfigure}{.45\textwidth}
  \centering
...
The detailed parameters are given in Section~\ref{section:SBM_parameters}.
Panel (a) shows the mean graph $P$ with $K = 5$ blocks and $N=200$ vertices and Panel (b) shows an adjacency matrix $A$ sampled according to the probabilities from $P$.
...
Based on three graphs sampled independently and identically according to the probability matrix $P$, we construct the element-wise mean $\bar{A}$, shown in Panel (d) (see Section~\ref{sec:abar}).  
Finally, by taking a rank-5 approximation of $\bar{A}$ and thresholding the values to be between $0$ and $1$, we construct our proposed estimate $\hat{P}$, shown in Panel (c) (see Section~\ref{sec:phat}).
...
In this section, we will illustrate the theoretical results from Section~\ref{section:theoretical_result} regarding the relative efficiency between $\bar{A}$ and $\hat{P}$ via Monte Carlo simulation experiments in an idealized setting.
...
\[
    \hat{\mathrm{RE}}_{st}(\bar{A},\hat{P}) = \frac{\sum_{\{(i, j): \tau_i=s,\tau_j=t,1 \le i < j \le n\}} \hat{\mathrm{MSE}}(\hat{P}_{ij})}{\sum_{\{(i, j): \tau_i=s,\tau_j=t,1 \le i < j \le n\}} \hat{\mathrm{MSE}}(\bar{A}_{ij})}
\]
...
We omit confidence intervals (calculated by assuming a normal distribution) for the estimated relative efficiencies since all confidence intervals have lengths less than $0.015$, indicating that all relative efficiencies, aside from the relative efficiency for the CPAC200 atlas at $M=10$, are very different from 1.
...
As discussed in Remark \ref{remark:low_rank}, we first check whether the dataset has the low-rank property or not. In Fig.~\ref{fig:screeplot}, we plot the relative error $\|\mathrm{lowrank}_d(P)-P\|_F^2/\|P\|_F^2$ of using a rank-$d$ approximation of $P$ (see Algorithm~\ref{algo:lowrank}) as solid curves {\color{red} for this plot i think we used negative eigenvalues ?}.
...
In previous sections, we have shown that the low-rank methods help us improve the accuracy of estimation in the small sample size.
In this section we investigate how the low-rank approximation of $P$, as represented by the embedded latent position, is related to the lobe-structure of the brain.

The lobes of the brain provide an anatomical classification, which has been shown to be related to different brain functions. In each hemisphere, there are four lobes: Frontal, Parietal, Occipital, and Temporal.
For the Desikan atlas, there are 70 different regions (35 regions for each hemisphere). 
Most regions belongs to one lobe, however, 8 regions of the Desikan atlas (Unknown, Banks of Superior Temporal Sulcus, and Corpus Callosum, for both hemispheres) do not have obvious lobe assignments, so we cluster them into a lobe category named ``other'' to resolve this issue.
...
\]
{\color{red} why not ratio? or an F statistic?}
...
In this situation, we can focus on how much of the lobe structure is really captured by the low-rank methods without affected by the inherent spatial relationship. Note that this test is underpowered compared to the test on a unconditional independence.
To test under the anatomical geometry conditions, we permute the lobe assignment $l$ in a way that the number of regions in each lobe remain the same and the regions within the same lobe are still spatially connected. In order to permute under such constraints, we define a flip to be a swap of two pairs of vertices which keeps the number of regions in each lobe, and do it several times. Details of the flipping procedure is discussed in Appendix~\ref{section:testing}. For a fixed number of flips, we ran 1000 simulations and calculate the test statistics $T(X, l)$ after the permutation. We vary the number of flips from 1 to 10 and the results are shown in Fig.~\ref{fig:violin_plot}. In this violin plot, we use dashed line to represent the $T(X, l)$ based on the true lobe assignment. As the number of flips increases, $T(X, l)$ converges to the expected value under the assumption that $X$ and $l$ are independent in a constrained way mentioned above. And we can see that the $T(X, l)$ according to the true lobe assignment move away from the 95\% confidence interval based on the permutations. We also calculate the corresponding p-values and labeled them in the figure. We can see that when the number of flips is larger than 7, the p-value is less than 0.05, which suggests that the latent positions based on the low-rank methods preserves the lobe structure regardless of the anatomical geometry.

\begin{figure}[!htbp]
\centering
\includegraphics[width=1\textwidth]{violinplot_new_flip_2norm_1_8.pdf}
\caption{{\bf Violin plot for testing lobe structure.}
We run 1000 simulations for each number of flips of lobe labels. The dashed line represents the value of $T(X,l_{true})$, based on true lobe assignment. 
As the number of flips increases, $T(X, l_{perm})$ converges to the {\color{red} expected value under the assumption that $X$ and $l$ are independent in a constrained way mentioned above}. And we can see that the $T(X, l)$ according to the true lobe assignment move away from the 95\% confidence interval.}
...
To further illuminate this point, we performed a synthetic data analysis under a full-rank independent edge model. As discussed in Section~\ref{sec:challenge}, the sample mean of the 454 graphs in the Desikan dataset is actually of full rank and will be considered as the probability matrix $P$ in this experiment.
We generated the synthetic data set of size $M = 1, 5$, and $10$ by sampling graphs independently from the full rank IEM with the probability matrix $P$. Then as in the previous section,  we calculated $\bar{A}$ and $\hat{P}$ of different embedding dimensions.
...
%\begin{figure}[!htbp]
%\centering
%\includegraphics[width=.7\textwidth]{eigenvector.pdf}
...
While in this paper we focus on the estimation of the probability matrix $P$ based on the low-rank structure, a lot of future directions are quite interesting, such as fitting the SBM or clustering the vertices to detect different brain regions.
For example, \citet{abraham2013extracting} introduced a region-extraction approach based on a sparse penalty with dictionary learning; and \citet{calhoun2001method} used ICA of fMRI data to draw group inferences. In future work, it would be interesting to explore further statistical inferences based on our method.
...
For this reason, we used the 3rd elbow in the experiments performed for this work. While \citet{zhu2006automatic} only defines the 1st elbow, we define the $s$-th elbow as in Algorithm~\ref{algo:ZG}.
...
\STATE Calculate the 1st elbow $d_1$ based on $\lambda_1, \dots, \lambda_N$ according to \citep{zhu2006automatic}
\FOR{$i$ = 2 to s}
	\STATE {Calculate the $i$-th elbow $d_i$ based on $\lambda_{d_{i - 1} + 1}, \dots, \lambda_N$ according to \citep{zhu2006automatic}}
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\begin{theorem}[Corollary of Theorem 1 in \citet{athreya2016limit}]\label{thm:clt_ext}
...
\section{Stochastic Blockmodel Parametere Setting}
\label{section:SBM_parameters}
Here we give the parameters in the stochastic blockmodel example depicted in Figure~\ref{fig:SBM_example}.
