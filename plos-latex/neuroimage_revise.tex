...
\vspace*{-16pt}
\begin{center}
  \includegraphics[height=.35\linewidth]{desikan1a.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan1b.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan1c.png}
\end{center}
\end{subfigure}\\
\vspace*{5pt}
\begin{subfigure}[t]{.7\textwidth}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[height=.35\linewidth]{desikan2a.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan2b.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan2c.png}
\end{center}
\end{subfigure}\\
\vspace*{5pt}
\begin{subfigure}[t]{.7\textwidth}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[height=.35\linewidth]{desikan3a.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan3b.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan3c.png}
\end{center}
\end{subfigure}\\
\vspace*{5pt}
\begin{subfigure}[t]{.7\textwidth}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[height=.35\linewidth]{desikan4a.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan4b.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan4c.png}
\end{center}
\end{subfigure}\\
\vspace*{5pt}
\begin{subfigure}[t]{.7\textwidth}
...
\vspace*{-16pt}
\begin{center}
  \includegraphics[height=.35\linewidth]{desikan5a.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan5b.png}\hspace{-10pt}
  \includegraphics[height=.35\linewidth]{desikan5c.png}
\end{center}
\end{subfigure}\\
...
While our estimator $\hat{P}$ performs well when the sample size $M$ is small and the number of vertices $N$ is large, the CoRR dataset itself is pretty challenging for our low-rank method.
...
For all three atlases, the relative error is large unless $d$ is close to $N$, which indicates that $P$ is not low-rank. If $P$ actually has the low-rank property, the relative error plot will look like the dashed curves, where we revised $P$ to be low-rank by only keeping a few large eigenvalues. 
Thus the CoRR dataset is actually high-rank, which violates the low-rank assumption of our results and make this real data experiment preferable to the entry-wise sample mean $\bar{A}$ compared to the low-rank estimator $\hat{P}$.
...
With all these challenges mentioned above, our results show that when the sample size is relatively small, such as $M=1$ or $M=5$, and for the atlases with a larger number of vertices, $\hat{P}$ still gives a better estimate than $\bar{A}$ for the CoRR dataset.
...
Generally, the regions within the same lobe should be more similar compared to the regions across the lobes. In order to see whether the embedded latent positions preserve this property or not, we propose a permutation test. We define the test statistics $T$ to be the average differences between vertices within the same lobe minus the average differences between vertices across different lobes, i.e.
\[
T(X, l) = \frac{\sum_{i \ne j, l(i) = l(j)} \|X_i - X_j \|_2}{\sum_{i \ne j, l(i) = l(j)} 1} -
\frac{\sum_{i \ne j, l(i) \ne l(j)} \|X_i - X_j \|_2}{\sum_{i \ne j, l(i) \ne l(j)} 1},
\]
where $l(i)$ represents the lobe assignment for vertex $i$. A small test statistic $T(X, l)$ indicates that latent positions of the regions within the same lobe are closer compared to the ones across the lobes, which is evidence that the low-rank methods preserves the lobe structure.

If the latent positions $X$ and the lobe assignment $l$ are independent, then we expect $T(X, l)$ to be close to zero. However, we are more interested in the case that $X$ and $l$ are independent in a way that the number of regions in each lobe remain the same and are still spatially connected after permutations. In this situation, we can focus on how much of the lobe structure is really captured by the low-rank methods without affected by the inherent spatial relationship. To permute under such constraints, we define a flip to be a swap of two pairs of vertices which keeps the number of regions in each lobe. Details of the flipping procedure is discussed in Appendix~\ref{section:testing}. For a fixed number of flips, we ran 1000 simulations and calculate the test statistics $T(X, l)$ after the permutation. We vary the number of flips from 1 to 10 and the results are shown in Fig.~\ref{fig:violin_plot}. In this violin plot, we use dashed line to represent the $T(X, l)$ based on the true lobe assignment. As the number of flips increases, $T(X, l)$ converges to the expected value under the assumption that $X$ and $l$ are independent in a constrained way mentioned above. And we can see that the $T(X, l)$ according to the true lobe assignment move away from the 95\% confidence interval. We also calculate the corresponding p-value as in Fig.~\ref{fig:pvalue}. From the figure, we can see that when the number of flips is larger than 7, the p-value is less than 0.5, which suggests that the latent positions based on the low-rank methods preserves the lobe structure.
...
Here we discuss the details of the flipping procedure in the hypothesis testing mentioned in Section~\ref{section:lobe_structure}.

As mentioned before, we have 10 lobes and 70 vertices according to the Desikan atlas. The 70 vertices are connected spatially in some way. Let the adjacency matrix of these 70 vertices to be $A$. $A_{ij} = 1$ means vertex $i$ and vertex $j$ are spatially connected. We say vertex $j$ is a neighbor of vertex $i$ if $A_{ij} = 1$. We define $l_i$ be the lobe i.d. for vertex $i$.

Define a uniform $1$-flip to be:
\begin{itemize}
