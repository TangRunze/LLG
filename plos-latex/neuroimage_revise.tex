...
In this situation, we can focus on how much of the lobe structure is captured by the low-rank methods, taking into account the inherent spatial relationship.
Note that this test will have substantially less power as compared to a test with a null hypothesis of unconditional independence.

To test these hypotheses, we randomly permute the lobe assignment for each region, maintaining the number of regions in each lobe and spatially contiguity of the lobes, to get a permuted lobe labeling $l_{perm}$.
This ensures that the anatomical geometry is preserved when the lobe labels are permuted so that the permuted labels adhere to null hypothesis.
In order to permute under such constraints, we sequentially flip the labels for two pairs of spatially adjacent regions across lobe boundaries, maintaining spatial contiguity and lobe size. Details of the flipping procedure are provided in Appendix~\ref{section:testing}. 

For a fixed number of flips, we ran 1000 simulations and calculate the test statistics $T(X, l_{perm})$ after the permutation. We vary the number of flips from 1 to 10 and the results are shown in Fig.~\ref{fig:violin_plot}. In this violin plot, we use dashed line to represent the $T(X, l_{true})$ based on the true lobe assignment.
As the number of flips increases, $T(X, l_{perm})$ converges to the expected value under the assumption that $X$ and $l$ are independent in a constrained way mentioned above. 
{\color{red} What is this expected value?}
When the number of flips is greater than 7, the p-value for this test goes below the 0.05 significance level, as calculated by the proportions of permutations with $T(X,l_{perm})<T(X,l_{true})$.
Overall, this shows that, from the perspective of the random dot product graph model, the lobe structure of the brain is statistically significant for the connectivity structure of the brain.
